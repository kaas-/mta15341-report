...
\centering
	\includegraphics[width=0.3\textwidth]{UnSub}
		\caption{The input image\label{Fig:UnSub}}
	\end{figure}
	
	\begin{figure}
	\centering
	\includegraphics[width=0.3\textwidth]{BackgroundSub}
		\caption{The background subtracted image\label{Fig:BackgroundSub}}
	\end{figure}
...
Since this project deals with video as opposed to static images, a single threshold with a fixed threshold value is not useful. As described in Section~\ref{sec:RDI}, the hardware setup uses RDI with IR light sources, and an IR filter on a web camera. 
