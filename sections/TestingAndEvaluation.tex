...
\end{enumerate}
	\item \begin{enumerate}
		\item Number of restarts of code during technical tests is logged (Unrelated to requirements. This is included as a way to track how autonomous\todo{"how automatic the code could function"? Not sure what is being said here  // Edit: Changed to "autonomous" MN} the code could be during the tests.)
	\end{enumerate}
	\item \begin{enumerate}
		\item Accuracy for tile changes made by hand (related to \ref{req:1B})
		
		This is done by pressing 5 tiles (4 corners and 1 centre tile) individually by hand and see if it was successful. Successful in this case means that the tile was able to be changed by hand from either side of the table. \todo{this needs to be clarified. Are they pressed at once? Are we testing whether the right one is terraformed? // Elaborated on both gripes MN}
	\end{enumerate}
\end{enumerate}
