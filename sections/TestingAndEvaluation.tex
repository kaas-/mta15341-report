...
\begin{enumerate}
	\item \begin{enumerate}
		\item Effectiveness of each tile press (related to requirement 			\ref{req:1B})
			\begin{enumerate}
				\item This is tested by pressing each of the 112 hexagons on the board by hand, and if hand would fail, then with a hexagonically shaped paper template.
			\end{enumerate}
		\item Efficiency of colour change (related to requirement \ref{req:1D})
		\begin{enumerate}
				\item This is tested by timing the moment of table contact to the tile colour change in seconds.
		\end{enumerate}
		\item Hand or template used (related to requirement \ref{req:1B})
		\begin{enumerate}
				\item It is registered whether or not hand gesture would work for changing tiles. The output is used for generating a map of successes and fails to see hard-to-detect areas. Figure \ref{fig:successfailmap} illustrates this.
		\end{enumerate}
...
\end{figure}
	...
	\item \begin{enumerate}
		...
		\begin{enumerate}
				\item This is tested by using paper cut-outs of hands to test successful detection of the respective players.
		\end{enumerate}
